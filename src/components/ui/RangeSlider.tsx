...
export const RangeSlider = React.forwardRef<HTMLInputElement, RangeSliderProps>(
  ({ className, label, value, min, max, unit, valueLabel, description, ...props }, ref) => {
      ...
      <div className={cn("w-full space-y-3", className)}>
        <div className="flex justify-between items-end">
          <div className="flex flex-col">
              ...
              {description && (
                  <span className="text-xs text-[#8A817C] mt-0.5 max-w-[200px] leading-tight">{description}</span>
              )}
          </div>
          <div className="text-right">
            <span className="text-2xl font-serif text-[#1F1F1F] font-medium">{value}</span>
            {unit && <span className="text-xs text-[#8A817C] ml-1 uppercase font-bold">{unit}</span>}
