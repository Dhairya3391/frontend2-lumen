...
export function Dashboard({
  ...
  const riskColor =
    currentResult.risk_level === "High"
      ? "#FF4D8C"
      : currentResult.risk_level === "Moderate"
        ? "#F59E0B"
        : "#10B981";

  // Calculate difference from original
  const riskDiff = Math.round(
    (currentResult.probability - initialResult.probability) * 100,
  );
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <motion.div
            ...
            className="bg-white rounded-[2.5rem] p-8 lg:p-12 shadow-sm border border-white relative overflow-hidden"
          >
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="text-center md:text-left">
                ...
                <p className="text-[#8A817C] max-w-xs mx-auto md:mx-0 text-lg">
                  Estimated based on your biometrics and lifestyle factors
                  relative to healthy benchmarks.
                </p>
              </div>
              ...
              <div className="relative">
                ...
                <div className="w-64 h-64 lg:w-80 lg:h-80 rounded-full border-20 border-[#F5F5F5] flex items-center justify-center relative bg-white shadow-inner">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[#8A817C] font-bold uppercase tracking-widest mt-2">
                      Years Old
                    </p>
                  </div>
                </div>
                ...
                >
                  <span className="text-sm font-bold text-[#1F1F1F] whitespace-nowrap">
                    Actual Age
                  </span>
                  <div className="w-px h-4 bg-gray-200"></div>
                  <span className="text-xl font-serif text-[#FF4D8C]">
                    {simulatedData.age}
                  </span>
                </motion.div>
              </div>
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-end gap-3 mb-8">
                  ...
                  {riskDiff !== 0 && (
                    <div
                      className={cn(
                        "flex items-center gap-1 mb-2 px-2 py-1 rounded-lg text-sm font-bold",
                        riskDiff > 0
                ...
                {[
                  {
                    ...
                    val: simulatedData.ap_hi > 130 ? "High" : "Optimal",
                    status: simulatedData.ap_hi > 130 ? "warning" : "good",
                  },
                  {
                    label: "BMI",
                    val: (
                      simulatedData.weight /
                      (simulatedData.height / 100) ** 2
                    ).toFixed(1),
                    status: "neutral",
                  },
                  {
                    label: "Lifestyle",
                    val: simulatedData.active ? "Active" : "Sedentary",
                    status: simulatedData.active ? "good" : "warning",
                  },
                ].map((item, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between p-4 rounded-2xl bg-[#F9F6F2]"
                  >
                    <span className="text-[#8A817C] font-medium">
                      {item.label}
                    </span>
                    <span
                      className={cn(
        ...
        <div className="lg:col-span-4">
          <motion.div
            variants={itemVariants}
            className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-pink-500/5 h-full border border-white sticky top-6"
          >
            <div className="flex items-center gap-3 mb-8">
              <div className="w-10 h-10 rounded-full bg-[#FFF0F5] flex items-center justify-center text-[#FF4D8C]">
                <RefreshCw
                  className={cn("w-5 h-5", isSimulating && "animate-spin")}
                />
              </div>
              <div>
                <h3 className="font-serif font-bold text-xl text-[#1F1F1F]">
                  Simulation
                </h3>
                <p className="text-xs text-[#8A817C] uppercase tracking-widest">
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-serif text-[#FF4D8C]">
                    {simulatedData.weight} kg
                  </span>
                </div>
                <RangeSlider
                  label=""
                ...
                <p className="text-xs text-[#8A817C]">
                  Reducing weight can significantly lower cardiovascular strain.
                </p>
              </div>
              ...
              <div className="space-y-4 pt-6 border-t border-[#F0F0F0]">
                ...
                <RangeSlider
                  ...
                  onChange={(e) =>
                    setSimulatedData({
                      ...simulatedData,
                      ap_hi: Number(e.target.value),
                    })
                  }
                />
                <p className="text-xs text-[#8A817C]">
                  Lower sodium intake and better sleep can improve this metric.
                </p>
              </div>

              <div className="space-y-4 pt-6 border-t border-[#F0F0F0]">
                <label className="text-sm font-bold text-[#1F1F1F] block mb-2">
                  Lifestyle Changes
                </label>
                ...
                <button
                  onClick={() =>
                    setSimulatedData((prev) => ({
                      ...prev,
                      smoke: prev.smoke ? 0 : 1,
                    }))
                  }
                  className={cn(
                    "w-full p-4 rounded-xl flex items-center justify-between border-2 transition-all duration-300",
                    simulatedData.smoke === 0
                      ? "border-[#10B981] bg-[#ECFDF5]"
